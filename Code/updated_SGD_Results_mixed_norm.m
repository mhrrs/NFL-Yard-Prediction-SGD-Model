...
function [msePerformance, rsePerformance, residuals] = lassoKCrossValidation(A, y, lam, k, t)
    ...
    for i = 1:k
        ...
        % SGD lasso
        if t == 2
            xh = lassoAlgSGD_2(A_train, y_train, lam, 1e-3, 100);
        end
    ...
    figure; 
    hold on; 
    plot(pred, 'ro', 'MarkerSize', 4); % Plot predictions in red
    plot(y_test, 'bo', 'MarkerSize', 4); % Plot actual values in blue
    hold off; % Release the figure
    xlabel('Data Point Index');
    ylabel('Yardage');
    legend('Predictions', 'Actual Values');
...
% I created another function, i wanna sleep
function xh = lassoAlgSGD_2(A, y, lam, learningRate, maxEpochs)     
    xnew = zeros(size(A,2),1);
    n = size(A,1);         

    for e = 1:maxEpochs
        perm = randperm(n);  % Shuffle the dataset
        ...
        for i = 1:n
            ...
            % Compute gradient without lasso
            gradient = -xi' * error;
            
            % Update coefficients
            xnew = xnew - learningRate * gradient;
            
            % Apply lasso penalty with soft thresholding
            for j = 1:length(xnew)
                if xnew(j) > lam * learningRate
                    xnew(j) = xnew(j) - lam * learningRate;
                elseif xnew(j) < -lam * learningRate
                    xnew(j) = xnew(j) + lam * learningRate;
                else
                    xnew(j) = 0;
                end
